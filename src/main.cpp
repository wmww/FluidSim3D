...
void init()
{
	...
	*/
	...
	sim.inflowDensity=0.8; //density to flow in
	sim.startVel=Vctr3<float>(3, 0, 0); //initial velocity
	...
	sim.voxDivisPerDensity=4; //how many particles to divide each density unit into
	sim.voxDivisMax=6; //max number of particles to divide the box into
	
	sim.randomMove=0.4; //randomness to add to the velocity
	//sim.randomMove=0.005; //randomness to add to the velocity
	//sim.randomMove=0; //randomness to add to the velocity
	sim.pressureForce=0.0001; //force of pressure
	//sim.pressureForce=0.01; //force of pressure
	//sim.pressureForce=0; //force of pressure
	sim.wallBounciness=1; //how bouncy walls are
...
void iteration()
{
	...
	{
		char text[100];
		sprintf(text, "bmp_otpt/img%d", sim.frame);
		sim.img.save(text);
	}
}
...
void displayData()
{
	...
	sprintf(text, "\nAvg frame Time:		%f\nAvg FPS:			%f\n",
			renderTime/sim.frame, sim.frame/renderTime);
	//sim.img.text(text, style1);
	window.text(text, style0);
